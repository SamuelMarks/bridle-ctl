...
describe('BatchJobDetailComponent', () => {
  ...
  it('should return correct badge variants for status', () => {
    const detailComponent = fixture.debugElement.query(
      By.directive(BatchJobDetailComponent),
    ).componentInstance;

    expect(detailComponent.getStatusVariant(BatchJobStatus.COMPLETED)).toBe(
      'success',
    );
    expect(detailComponent.getStatusVariant(BatchJobStatus.FAILED)).toBe(
      'danger',
    );
    expect(detailComponent.getStatusVariant(BatchJobStatus.RUNNING)).toBe(
      'accent',
    );
    expect(detailComponent.getStatusVariant(BatchJobStatus.PENDING)).toBe(
      'default',
    );
    expect(detailComponent.getStatusVariant(BatchJobStatus.INTERRUPTED)).toBe(
      'default',
    );
  });
});
